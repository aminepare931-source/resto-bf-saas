...
export function DishModal({
  ...
  return (
    <div
      className="fixed inset-0 z-[300] flex items-center justify-center p-4 sm:p-5"
    ...
    >
      <div
        className="w-full max-w-md rounded-[20px] overflow-hidden max-h-[90vh] overflow-y-auto shadow-2xl border transition-all"
      ...
      >
        <div className="relative h-52 sm:h-60" style={{ background: theme.surfaceAlt }}>
          ...
          <button
            onClick={onClose}
            aria-label="Fermer"
            className="absolute top-3 right-3 w-8 h-8 rounded-full grid place-items-center text-sm font-bold shadow-md hover:scale-105 transition"
        ...
        <div className="p-6">
          <span
            className="inline-block px-2.5 py-1 text-[10px] font-bold uppercase tracking-widest rounded mb-2"
          ...
          <div className="flex gap-3">
            {cart ? (
              <button
                onClick={() => {
                  ...
                  toast.success(`${dish.name} (${qty}) ajouté au panier !`);
                  onClose();
                }}
                className="flex-1 text-center py-3 px-4 rounded-xl font-bold text-sm shadow-md hover:opacity-90 active:scale-95 transition"
            ...
            <button
              onClick={onClose}
              className="px-5 py-3 rounded-xl font-bold text-sm border transition hover:bg-black/5"
...
export function MenuGrid({ menu, theme }: { menu: PublicMenuItem[]; theme: Theme }) {
  ...
  const groups = groupByCategory(filteredMenu);
      ...
      <div className="space-y-4">
        <div className="relative max-w-md mx-auto">
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 rounded-full text-sm outline-none transition"
            style={{
          ...
          {search && (
            <button
              onClick={() => setSearch("")}
              className="absolute right-3 top-2.5 text-xs px-1.5 py-0.5 rounded-full opacity-60 hover:opacity-100"
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none justify-start sm:justify-center">
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-xs font-bold whitespace-nowrap transition-all ${
                selectedCategory === "all" ? "shadow-md scale-105" : "opacity-75 hover:opacity-100"
              }`}
              style={{
                background: selectedCategory === "all" ? theme.accent : theme.surfaceAlt,
                color: selectedCategory === "all" ? theme.accentInk : theme.text,
                border: `1px solid ${theme.border}`,
              }}
            >
              Tout ({menu.filter((m) => m.available).length})
            </button>
            {categories.map((cat) => {
              const count = menu.filter((m) => m.available && m.category === cat).length;
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                {items.map((it) => (
                  <div
                    key={it.id}
                    onClick={() => setOpen(it)}
                    className="group relative flex gap-4 p-4 rounded-2xl text-left w-full cursor-pointer hover:-translate-y-1 transition-all duration-200 border"
                    style={{
                      background: theme.surface,
                  ...
                  >
                    {it.image_url ? (
                      <div
                        className="w-20 h-20 sm:w-24 sm:h-24 shrink-0 overflow-hidden rounded-xl"
                    ...
                    <div className="flex-1 min-w-0 flex flex-col justify-between">
                      <div>
                        <div className="flex items-start justify-between gap-2 mb-1">
                          ...
                          <span
                            className="font-black whitespace-nowrap text-sm shrink-0"
                        ...
                        {cart && (
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              cart.addItem(it.id);
                              toast.success(`${it.name} ajouté au panier !`);
                            }}
                            className="px-2.5 py-1 rounded-lg text-xs font-bold transition hover:scale-105 active:scale-95 shadow-sm"
                            style={{
...
export function GalleryGrid({ gallery, theme }: { gallery: PublicGalleryImage[]; theme: Theme }) {
  const [selectedPhoto, setSelectedPhoto] = useState<PublicGalleryImage | null>(null);
      ...
      <div className="columns-2 md:columns-3 lg:columns-4 gap-3 space-y-3">
        {gallery.map((g, i) => (
          <div
            ...
            className="break-inside-avoid overflow-hidden cursor-pointer group relative rounded-xl transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5"
            style={{
      ...
      {selectedPhoto && (
        ...
        >
          <div className="relative max-w-4xl w-full max-h-[90vh] flex flex-col items-center justify-center">
            <button
              onClick={() => setSelectedPhoto(null)}
              className="absolute -top-10 right-0 text-white text-xl font-bold px-3 py-1 rounded-full bg-white/20 hover:bg-white/40 transition"
            >
              ✕
            </button>
            <StorageImage
              path={selectedPhoto.image_url}
              alt={selectedPhoto.caption ?? "Photo"}
              className="max-w-full max-h-[80vh] object-contain rounded-xl shadow-2xl"
            />
            {selectedPhoto.caption && (
              <p className="mt-3 text-white text-center text-sm font-medium bg-black/50 px-4 py-2 rounded-lg">
                {selectedPhoto.caption}
              </p>
            )}
          </div>
...
/* ---------- Standard Mobile Floating Bar ---------- */

export function StandardMobileBar({
  restaurant,
  waLink,
  theme,
  onOpenCart,
  ...
  return (
    ...
    >
      ...
      {cart && (
        <button
          onClick={onOpenCart}
          className="flex flex-col items-center gap-0.5 text-[11px] font-bold relative"
          style={{ color: theme.text }}
        >
...
export function ReservationForm({ restaurantId, theme }: { restaurantId: string; theme: Theme }) {
      ...
      {done ? (
        ...
        >
          ...
          <p style={{ color: theme.textMuted, marginBottom: 20 }}>
            Le restaurant vous confirmera rapidement.
          </p>
          <button
            onClick={reset}
            style={{
        ...
        <form
          ...
          className="space-y-4"
          style={{ background: theme.surface, borderRadius: theme.radius, padding: 28 }}
        >
          <p
            style={{
              fontSize: 10,
            ...
            {times.map((t) => (
              <button
                ...
                style={{
                  ...
                  fontWeight: 700,
                  cursor: "pointer",
                  border: `1px solid ${time === t ? theme.accent : theme.border}`,
                  background: time === t ? theme.accent : "transparent",
                  color: time === t ? theme.accentInk : theme.text,
                }}
              >
                {t}
              </button>
            ...
            {guestList.map((g) => (
              <button
                ...
                style={{
                  ...
                  fontWeight: 700,
                  cursor: "pointer",
                  border: `1px solid ${guests === g ? theme.accent : theme.border}`,
                  background: guests === g ? theme.accent : "transparent",
                  color: guests === g ? theme.accentInk : theme.text,
                }}
              >
                {g} pers.
              </button>
          ...
          <div className="flex flex-wrap gap-2">
            {occasions.filter(Boolean).map((o) => (
              <button
                type="button"
                key={o}
                onClick={() => setForm({ ...form, occ: form.occ === o ? "" : o })}
                style={{
                  padding: "8px 16px",
                  borderRadius: 20,
                  fontSize: 12,
                  fontWeight: 600,
                  cursor: "pointer",
                  border: `1px solid ${form.occ === o ? theme.accent : theme.border}`,
                  background: form.occ === o ? theme.accent : "transparent",
                  color: form.occ === o ? theme.accentInk : theme.text,
                }}
              >
                {o}
              </button>
...
const ICON_PATHS: Record<string, string> = {
  ...
  briefcase:
    "M4 8h16v11a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1V8Zm4 0V6a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2M4 13h16",
  ...
  calendar:
    "M7 3v3M17 3v3M4 8h16M5 6h14a1 1 0 0 1 1 1v12a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1V7a1 1 0 0 1 1-1Zm2 7h3m-3 4h7",
  phone:
    "M6.5 3h3l1.5 4-2 1.5a12 12 0 0 0 5.5 5.5l1.5-2 4 1.5v3a2 2 0 0 1-2 2C11.5 20 4 12.5 4 5a2 2 0 0 1 2.5-2Z",
  ...
  building:
    "M6 21V5a1 1 0 0 1 1-1h10a1 1 0 0 1 1 1v16M6 21h12M6 21H4m14 0h2M9 8h1m4 0h1M9 12h1m4 0h1M9 16h1m4 0h1",
};
