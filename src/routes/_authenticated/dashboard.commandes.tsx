...
function OrdersPage() {
      ...
      .on(
        ...
        (payload) => {
          if (payload.eventType === "INSERT") {
            ...
            setOrders((prev) =>
              prev.map((o) =>
                o.id === (payload.new as Order).id ? (payload.new as unknown as Order) : o,
              ),
            );
  ...
  const sendToKitchen = async (order: Order) => {
    ...
    const channel = order.restaurant_id
      ? (
          await supabase
            .from("restaurants")
            .select("notification_orders_channel")
            .eq("id", order.restaurant_id)
            .maybeSingle()
        )?.data?.notification_orders_channel || "both"
      : "both";
  ...
  const counts = useMemo(() => {
    ...
    for (const s of Object.keys(ORDER_STATUS_LABEL))
        ...
        <button
          ...
          title={
            voiceMutedState
              ...
              : "Couper l'annonce vocale des commandes"
          }
        >
          {voiceMutedState ? <VolumeX className="w-4 h-4" /> : <Volume2 className="w-4 h-4" />}
          <span className="hidden sm:inline">
            {voiceMutedState ? "Voix coupée" : "Voix activée"}
          </span>
        </button>
      </div>
