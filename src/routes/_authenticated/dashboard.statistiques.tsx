...
function StatisticsPage() {
  ...
  const todayChange =
    stats.yesterday > 0 ? ((stats.today - stats.yesterday) / stats.yesterday) * 100 : 0;
  const weekChange =
    stats.lastWeek > 0 ? ((stats.week - stats.lastWeek) / stats.lastWeek) * 100 : 0;
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {peakHours.length > 0 && topItems.length > 0 ? (
                  <>
                    ...
                    commandes. Assurez-vous d'avoir suffisamment de personnel et de stock pour votre
                    article le plus populaire : <strong>{topItems[0].description}</strong>.
                  </>
                ) : (
