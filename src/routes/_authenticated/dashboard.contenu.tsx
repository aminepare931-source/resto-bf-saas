  ...
  useEffect(() => {
    (async () => {
      ...
      const { data } = await supabase
        .from("restaurants")
        .select(
          "id, logo_url, hero_title, hero_subtitle, about_text, primary_color, font_family, invoice_prefix, invoice_footer, social_links",
        )
      ...
      <form onSubmit={save} className="space-y-8">
        ...
        <Section title="Factures (Premium)">
          ...
          <TextArea
            ...
            onChange={(v) => setForm((f) => ({ ...f, invoice_footer: v }))}
            rows={2}
            placeholder="Merci de votre visite — RCCM BFOUA..."
          />
        </Section>
