    ...
    return (
      <div className="max-w-2xl p-8 rounded-3xl border border-gold/30 bg-gradient-to-br from-gold/10 to-transparent text-center">
        ...
        <p className="mt-3 text-sm text-muted-foreground">
          Créez et imprimez vos factures (PDF) avec votre logo, votre numérotation, le suivi payé /
          impayé. Réservé à l'abonnement Premium.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-6xl">
              ...
              {invoices.map((inv) => (
                <tr key={inv.id} className="border-t border-white/5">
                  ...
                  <td className="p-4 text-right font-bold">
                    {inv.total.toLocaleString("fr-FR")} F
                  </td>
                  <td className="p-4">
                    <StatusBadge
                      status={inv.status}
      ...
      {creating && r && (
        <InvoiceModal
          restaurantId={r.id}
          prefix={(r as any).invoice_prefix ?? "FACT"}
...
function InvoiceModal({
  ...
  const submit = async () => {
    ...
    if (lines.some((l) => !l.description))
      return toast.error("Toutes les lignes doivent être décrites");
    setSaving(true);
