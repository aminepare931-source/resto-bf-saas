...
export function announceNewOrder(opts: {
  ...
  if (opts.items.length > 5)
    parts.push(
      `et ${opts.items.length - 5} autre${opts.items.length - 5 > 1 ? "s" : ""} article${opts.items.length - 5 > 1 ? "s" : ""}.`,
    );
  parts.push(`Total : ${Math.round(opts.total).toLocaleString("fr-FR")} francs.`);
  speak(parts.join(" "));
}

export function announceNewReservation(opts: {
