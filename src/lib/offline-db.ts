...
type StoreName =
  "menu_items" | "categories" | "restaurants" | "settings" | "pages" | "images" | "pending_sync";
...
export async function cacheRestaurantData(
  restaurantId: string,
  data: {
  ...
  if (data.menu_items) {
    await cacheData(
      "menu_items",
      data.menu_items.map((item) => ({ ...item, _restaurantId: restaurantId })),
    );
  }
  if (data.categories) {
    await cacheData(
      "categories",
      data.categories.map((cat) => ({ ...cat, _restaurantId: restaurantId })),
    );
  }
  if (data.restaurant) {
...
export async function getCachedMenuItems(restaurantId: string): Promise<Record<string, unknown>[]> {
  const all = (await getCachedData<Record<string, unknown>>("menu_items")) as Record<
    string,
    unknown
  >[];
...
export async function getCachedCategories(
  ...
  const all = (await getCachedData<Record<string, unknown>>("categories")) as Record<
    string,
    unknown
  >[];
