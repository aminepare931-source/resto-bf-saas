...
export const standardTemplate: InvoiceTemplate = {
  ...
  render: (data, colors) => ({
    pdf: (doc) => {
      ...
      doc.setFont("helvetica", "normal");
      doc.text(
        `Date: ${new Date(data.issuedAt).toLocaleDateString("fr-FR")}`,
        pageWidth - margin,
        32,
        { align: "right" },
      );
      if (data.dueAt) {
        doc.text(
          `Échéance: ${new Date(data.dueAt).toLocaleDateString("fr-FR")}`,
          pageWidth - margin,
          38,
          { align: "right" },
        );
...
export const premiumTemplate: InvoiceTemplate = {
  ...
  render: (data, colors, logoUrl) => ({
    pdf: (doc) => {
      ...
      const bgColor =
        colors.background === "#ffffff" ? 245 : parseInt(colors.background.replace("#", ""), 16);
  ...
  render: (data, colors) => ({
    pdf: (doc) => {
      ...
      autoTable(doc, {
        ...
        foot: [["", "", "TOTAL", `${data.total.toLocaleString("fr-FR")} F`]],
        footStyles: {
          fillColor: false,
          textColor: [0, 0, 0],
          fontStyle: "bold",
  ...
  render: (data, colors) => ({
    pdf: (doc) => {
      ...
      autoTable(doc, {
        ...
        foot: [["", "", `TOTAL PAYÉ: ${data.total.toLocaleString("fr-FR")} F`]],
        footStyles: {
          fillColor: primaryRgb,
          textColor: [255, 255, 255] as [number, number, number],
          fontStyle: "bold",
