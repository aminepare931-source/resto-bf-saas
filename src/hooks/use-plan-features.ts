...
export async function updatePlanFeaturePlans(featureId: string, plans: string[]): Promise<boolean> {
...
export async function addPlanFeature(
  ...
  const { error } = await supabase.from("plan_features" as never).insert(feature as never);
